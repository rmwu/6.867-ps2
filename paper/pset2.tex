...
% TODO phrase as prose:
\begin{itemize}
    \item
    Gaussian kernels produced better classification accuracy, but there were more support vectors.
    Intuition: in higher dimensions, supporting hyperplanes rest on more support vectors.
    \item
    For linearly inseparable data, and linear kernel, changing $C$ didn't change much. (dataset 2)
    \item
    small bandwidth lets you pick out fine structure (dataset 2, sigma = 0.5, C = 100)
    \item
    Gaussian kernels don't overfit super-easily (unless you pick small bandwidth)
\end{itemize}
