...
We observed that $||w||$ decreases, eventually converging, over time.
...
After training, we used our validation set to select the optimal hyperparameters for each dataset (Table \ref{table:regularization-four}).
Some datasets did not have a unique optimal parameter; for these, we arbitrarily selected one.
These values were then to obtain the performance on our test sets (Table \ref{table:1-3-optimal}).
