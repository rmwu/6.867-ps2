...
After training, we used our validation set to select the optimal parameters for each dataset. We noticed that some datasets did not have a unique optimal parameter; for these, we arbitrarily selected one. These values were then to obtain the performance on our test sets (Table \ref{table:1-3-optimal}).
...
\begin{equation}
    K(x, y) = e^{-\gamma || x-y || ^2}
\end{equation}
We found that smaller values of $\gamma$ resulted in fewer support vectors.
...
\end{tabular}
\end{center}
Thus, the decision boundary becomes more tight as we decrease $\gamma$.

% 2^{-2} & 2^{-1} & 1 & 2 & 4
 % 1/400 & 1/400 & 2/400 & 45/400 & 100/400
Given the support vector values $\alpha$, we can make predictions for new inputs $x$ by checking if 
\begin{equation}
    \sum_j{a_j K(x^{(i)},x^{(j)}) < 1 }
\end{equation}
This formulation is a soft-SVM, so points may be inside the margins.
